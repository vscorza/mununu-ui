...
const successInterceptor = (response: AxiosResponse) => {
  ...
  const correlationId =
    response.headers["x-correlation-id"] || config.headers["X-Correlation-ID"];
  if (correlationId && import.meta.env.DEV) {
    console.log(
      `[API] ${method} ${endpoint} - Correlation ID: ${correlationId}`,
    );
  }
  ...
  (response as AxiosResponse & { correlationId?: string }).correlationId =
...
const errorInterceptor = (error: AxiosError) => {
  ...
  const key = config
    ? `${config.method || "get"}_${config.url || "unknown"}`
    : "unknown";
  const startTime = requestStartTimes.get(key);
  const duration = startTime ? performance.now() - startTime : 0;
  requestStartTimes.delete(key); // Clean up
