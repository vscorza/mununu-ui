...
class AnalyticsService {
  ...
  logError(
    ...
    console[
      severity === "error" ? "error" : severity === "warning" ? "warn" : "log"
    ](`[${emoji} Analytics] ${source}: ${message}`, context || "");
  }
  ...
  getSummary(): {
    ...
    const successfulApiCalls = this.apiCalls.filter(
      (call) => call.success,
    ).length;
    const failedApiCalls = this.apiCalls.filter((call) => !call.success).length;
    const totalDuration = this.apiCalls.reduce(
      (sum, call) => sum + call.duration,
      0,
    );
    const averageDuration =
      this.apiCalls.length > 0 ? totalDuration / this.apiCalls.length : 0;

    const errors = this.errorLogs.filter(
      (log) => log.severity === "error",
    ).length;
    const warnings = this.errorLogs.filter(
