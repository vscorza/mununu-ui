...
const processQueuedOperation = async (operation: QueuedOperation): Promise<boolean> => {
  ...
  try {
    const client = operation.endpoint.includes('/ai/') ? aiApiClient : apiClient
    const method =
      (operation.method.toLowerCase() as 'get' | 'post' | 'put' | 'delete' | 'patch') || 'get'
    ...
    console.warn('[OfflineSync] Failed to sync operation:', operation.id, error)
    return false
  }
}
 ...
 */
export const syncQueuedOperations = async (): Promise<{ success: number; failed: number }> => {
  if (!navigator.onLine) {
  ...
  for (const operation of operations) {
    ...
    if (result) {
      ...
      if (operation.maxRetries && operation.retries >= operation.maxRetries) {
        offlineQueue.dequeue(operation.id)
        failed++
        console.warn('[OfflineSync] Max retries exceeded for operation:', operation.id)
      }
    }
